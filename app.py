# app.py - EcoSense AI (restructured, fixed, enhanced)
# Features:
...
with st.sidebar:
    ...
    try:
        ...
        st.markdown("Drive the EcoJourney")

    st.markdown("---")
    st.header("Settings")

    # Mode and user
    mode = st.radio("Mode", ["🚗 EV User", "🚙 Non-EV User"])
    ...
    st.markdown("---")
    get_plan = st.button("📈 Get Renewable-aware Charging Plan")
    gen_report_btn = st.button("📄 Generate Monthly PDF Report")
...
# ---------------- Header ----------------
st.markdown("<h1 style='text-align:center; color:#1b5e20;'>EcoSense AI</h1>", unsafe_allow_html=True)
st.markdown("<div style='text-align:center; color:#555;'>Drive the EcoJourney — renewable-aware EV charging & eco coaching</div>", unsafe_allow_html=True)
st.write("---")
...
with col_main:
    ...
    if mode.startswith("🚗"):
        ...
        st.write("We compute a green score (solar + wind) for forecast hours and recommend best charging window.")
            ...
            try:
                ...
                if df_before.empty:
                    ...
                    if best_start is not None:
                        ...
                        st.success(f"Recommended charging window (UTC): **{fmt_dt(best_start)} → {fmt_dt(best_end)}** (avg green score {best_avg:.2f})")
                        ...
                        ax.plot(plot_df.index, plot_df["green_score"], marker='o', label='green_score')
                        rec = plot_df[plot_df["recommended"] == 1]
                        ax.scatter(rec.index, rec["green_score"], s=100, label='recommended')
                        ...
                        st.table(preview[cols_show].head(24))
                    else:
                        st.warning("Couldn't find contiguous block matching required charging duration.")
            except Exception as e:
                st.error("Failed to fetch or process forecast: " + str(e))

        st.markdown("**Tip:** If you have rooftop solar, daytime charging (11:00–15:00 local) is usually best. Wind peaks depend on location.")
    else:
        # ---------- Non-EV coach ----------
    ...
    st.caption("Try natural language scenarios: 'bike 6 km 5 days/week', 'skip 1 flight 1200 km', 'install 3 kW solar', 'replace 8 bulbs with LED', 'wfh 2 days/week', 'raise AC by 2C'")
        ...
        if st.button(label):
            if not username:
                ...
                users_df.loc[users_df["username"] == username, "points"] = int(
                    users_df.loc[users_df["username"] == username, "points"].values[0]) + pts
                users_df.to_csv(DATA_PATH, index=False)
...
with col_side:
    ...
    if not lb.empty:
        st.table(lb[["username", "vehicle_type", "points"]])
    else:
    ...
    if gen_report_btn:
        if not username:
            ...
            try:
                ...
                c.save()
                buf.seek(0)
                st.download_button("📥 Download Monthly Report (PDF)", buf, file_name=f"{username}_ecosense_report.pdf", mime="application/pdf")
            except Exception as e:
...
# ---------------- Footer / notes ----------------
st.write("---")
st.markdown("**Notes:** Green score is a simple normalized mix of solar radiation and wind speed from Open-Meteo (free). For higher accuracy integrate inverter or grid carbon intensity APIs (WattTime / ElectricityMap).")
